...
def majorvote(neighbors, weighted=False):
    freq = {}
    #for features, dist, label in neighbors:
     #   if weighted and dist != 0:
      #      weight = 1 / (dist ** 2)  # Use 1/d^2 for weighted voting
       # else:
        #    weight = 1  # Unweighted voting
...
def main():
    ...
    print("1.weighted voting 2,unweighted voting")
    weight = input("Enter choice")
    distance_table = []
    ...
    predicted_class, vote_count = majorvote(neighbors, weighted=weighted_voting)
    print("\nClass Frequency:", vote_count)


    if not weighted_voting:
        vote_count_int = {cls: int(count) for cls, count in vote_count.items()}
        if list(vote_count_int.values()).count(max(vote_count_int.values())) > 1:
        ...
        print("No neighbors selected, cannot predict class.")
         ...
         if list(vote_count.values()).count(max(vote_count.values())) > 1:
            print("Tie detected → Selected class of nearest neighbor (based on sorted distance)")
